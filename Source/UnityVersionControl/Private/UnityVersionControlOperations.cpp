...
bool FPlasticMakeWorkspaceWorker::Execute(FUnityVersionControlCommand& InCommand)
{
	...
	if (InCommand.bCommandSuccessful)
	{
		...
		InCommand.bCommandSuccessful = UnityVersionControlUtils::RunCommand(TEXT("workspace"), Parameters, TArray<FString>(), InCommand.InfoMessages, InCommand.ErrorMessages);
	}
...
bool FPlasticGetLocksWorker::Execute(FUnityVersionControlCommand& InCommand)
{
...

	{
		...
		InCommand.bCommandSuccessful = UnityVersionControlUtils::RunListLocks(GetProvider(), bForAllDestBranches, Operation->Locks);
	}
...
bool FPlasticUnlockWorker::Execute(FUnityVersionControlCommand& InCommand)
{
	...
	for (const FUnityVersionControlLockRef& Lock : Operation->Locks)
	{
...
{
	UnityVersionControlUtils::UpdateCachedStates(MoveTemp(States));
