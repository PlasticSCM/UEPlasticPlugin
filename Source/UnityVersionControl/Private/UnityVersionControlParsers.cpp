...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UnityVersionControlParsers::ParseFileStatusResult);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UnityVersionControlParsers::ParseDirectoryStatusResult);
...
public:
	explicit FPlasticFileinfoParser(const FString& InResult)
	{
		...
		if (Fileinfos.Num() == 6)
		{
			...
			LockedBy = UnityVersionControlUtils::UserNameToDisplayName(MoveTemp(Fileinfos[3]));
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UnityVersionControlParsers::ParseFileinfoResults);
	...
	if (Provider.GetPlasticScmVersion() >= UnityVersionControlVersions::SmartLocks)
	{
		// In the Content Browser, only show locks applying to the current working branch
		const bool bForAllDestBranches = false;
		UnityVersionControlUtils::RunListLocks(Provider, bForAllDestBranches, Locks);
	}
	...
	for (int32 IdxResult = 0; IdxResult < InResults.Num(); IdxResult++)
	{
		...
		const TArray<FUnityVersionControlLockRef> MatchingLocks = FindMatchingLocks(Locks, FileinfoParser.ServerPath);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UnityVersionControlParsers::ParseUpdateResultsString);
...
FText ParseCheckInResults(const TArray<FString>& InResults)
{
	if (InResults.Num() > 0)
	{
		...
		if (InResults.Last().StartsWith(ChangesetPrefix))
		{
			...
			return FText::Format(NSLOCTEXT("UnityVersionControl", "SubmitMessage", "Submitted changeset {0}"), FText::FromString(ChangesetString));
		}
		else
		{
...
bool ParseShelveDiffResult(const FString InWorkspaceRoot, TArray<FString>&& InResults, FUnityVersionControlChangelistState& InOutChangelistsState)
{
	...
	for (FString& Result : InResults)
	{
		...
		if (ShelveState != EWorkspaceState::Unknown && !Result.IsEmpty())
		{
			...
			UnityVersionControlUtils::AddShelvedFileToChangelist(InOutChangelistsState, MoveTemp(AbsoluteFilename), ShelveState, MoveTemp(MovedFrom));
		}
		else
		{
			bResult = false;
...
*/
static bool ParseBranchesResults(const FXmlFile& InXmlResult, TArray<FUnityVersionControlBranchRef>& OutBranches)
{
	static const FString PlasticQuery(TEXT("PLASTICQUERY"));
	static const FString Branch(TEXT("BRANCH"));
	static const FString Comment(TEXT("COMMENT"));
	...
	for (const FXmlNode* BranchNode : BranchsNodes)
	{
		...
		FUnityVersionControlBranchRef BranchRef = MakeShareable(new FUnityVersionControlBranch());
