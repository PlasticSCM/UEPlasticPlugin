...
void FUnityVersionControlProvider::Init(bool bForceConnection)
{
	...
	if (bForceConnection && bPlasticAvailable && bWorkspaceFound && !bServerAvailable)
	{
		...
		bServerAvailable = UnityVersionControlUtils::RunCheckConnection(WorkspaceSelector, BranchName, RepositoryName, ServerUrl, InfoMessages, ErrorMessages);
		if (!bServerAvailable)
		{
...
void FUnityVersionControlProvider::CheckPlasticAvailability()
{
	...
	if (!PathToPlasticBinary.IsEmpty())
	{
		...
		Profiles = UnityVersionControlUtils::GetProfiles();

		if (bWorkspaceFound)
		{
			TArray<FString> ErrorMessages;
			UnityVersionControlUtils::GetWorkspaceInfo(WorkspaceSelector, BranchName, RepositoryName, ServerUrl, ErrorMessages);
			UserName = UnityVersionControlUtils::GetProfileUserName(Profiles, ServerUrl);
		}
		else
		{
...
void FUnityVersionControlProvider::UpdateServerUrl(const FString& InServerUrl)
{
	ServerUrl = InServerUrl;
	UserName = UnityVersionControlUtils::GetProfileUserName(Profiles, InServerUrl);
}
