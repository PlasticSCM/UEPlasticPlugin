...
#define LOCTEXT_NAMESPACE "UnityVersionControl.State"

const TCHAR* FUnityVersionControlState::ToString() const
{
...
FText FUnityVersionControlState::ToText() const
{
...
int32 FUnityVersionControlState::GetHistorySize() const
{
	return History.Num();
}

TSharedPtr<class ISourceControlRevision, ESPMode::ThreadSafe> FUnityVersionControlState::GetHistoryItem(int32 HistoryIndex) const
{
	check(History.IsValidIndex(HistoryIndex));
	return History[HistoryIndex];
}

TSharedPtr<class ISourceControlRevision, ESPMode::ThreadSafe> FUnityVersionControlState::FindHistoryRevision(int32 RevisionNumber) const
{
...
TSharedPtr<class ISourceControlRevision, ESPMode::ThreadSafe> FUnityVersionControlState::FindHistoryRevision(const FString& InRevision) const
{
	for (const auto& Revision : History)
	{
...
TSharedPtr<class ISourceControlRevision, ESPMode::ThreadSafe> FUnityVersionControlState::GetBaseRevForMerge() const
{
	for (const auto& Revision : History)
	{
...
TSharedPtr<class ISourceControlRevision, ESPMode::ThreadSafe> FUnityVersionControlState::GetCurrentRevision() const
{
	for (const auto& Revision : History)
	{
		// look for the changeset number, not the revision
...
ISourceControlState::FResolveInfo FUnityVersionControlState::GetResolveInfo() const
{
...
FName FUnityVersionControlState::GetIconName() const
{
	if (!IsCurrent())
	{
	...
	case EWorkspaceState::Changed: // Changed but unchecked-out file is in a certain way not controlled
		if (GetDefault<UUnityVersionControlProjectSettings>()->bPromptForCheckoutOnChange)
		{
			return FName("Perforce.NotInDepot");
		}
		else
...
FName FUnityVersionControlState::GetSmallIconName() const
{
	if (!IsCurrent())
	{
...
FSlateIcon FUnityVersionControlState::GetIcon() const
{
#if ENGINE_MINOR_VERSION >= 2
...
FText FUnityVersionControlState::GetDisplayName() const
{
...
FText FUnityVersionControlState::GetDisplayTooltip() const
{
	FNumberFormattingOptions NoCommas;
	NoCommas.UseGrouping = false;
	if (!IsCurrent())
...
const FString& FUnityVersionControlState::GetFilename() const
{
	return LocalFilename;
}

const FDateTime& FUnityVersionControlState::GetTimeStamp() const
{
	return TimeStamp;
}

bool FUnityVersionControlState::CanCheckIn() const
{
...
bool FUnityVersionControlState::CanCheckout() const
{
	if (!GetDefault<UUnityVersionControlProjectSettings>()->bPromptForCheckoutOnChange)
	{
		return false;
...
bool FUnityVersionControlState::IsCheckedOut() const
{
...
bool FUnityVersionControlState::IsLocked() const
{
	return !LockedBy.IsEmpty();
}

bool FUnityVersionControlState::IsRetainedInOtherBranch() const
{
	return !RetainedBy.IsEmpty();
}

bool FUnityVersionControlState::IsCheckedOutOther(FString* Who) const
{
	if (Who != NULL)
	{
		*Who = LockedBy;
...
bool FUnityVersionControlState::IsCheckedOutInOtherBranch(const FString& CurrentBranch /* = FString() */) const
{
...
bool FUnityVersionControlState::IsModifiedInOtherBranch(const FString& CurrentBranch /* = FString() */) const
{
...
*/
bool FUnityVersionControlState::GetOtherBranchHeadModification(FString& HeadBranchOut, FString& ActionOut, int32& HeadChangeListOut) const
{
...
bool FUnityVersionControlState::IsCurrent() const
{
...
bool FUnityVersionControlState::IsSourceControlled() const
{
...
bool FUnityVersionControlState::IsAdded() const
{
...
bool FUnityVersionControlState::IsDeleted() const
{
...
bool FUnityVersionControlState::IsIgnored() const
{
...
bool FUnityVersionControlState::CanEdit() const
{
...
bool FUnityVersionControlState::CanDelete() const
{
	return !IsCheckedOutOther() && IsSourceControlled() && IsCurrent();
}

bool FUnityVersionControlState::IsUnknown() const
{
	return WorkspaceState == EWorkspaceState::Unknown;
}

bool FUnityVersionControlState::IsModified() const
{
...
bool FUnityVersionControlState::CanAdd() const
{
...
bool FUnityVersionControlState::IsConflicted() const
{
...
bool FUnityVersionControlState::CanRevert() const
{
