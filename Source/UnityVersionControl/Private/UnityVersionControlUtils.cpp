...
TMap<FString, FString> GetProfiles()
{
	...
	if (bResult)
	{
		return UnityVersionControlParsers::ParseProfileInfo(Results);
	}
...
bool RunGetProjects(const FString& InServerUrl, TArray<FString>& OutProjectNames, TArray<FString>& OutErrorMessages)
{
	...
	if (bResult)
	{
		OutProjectNames = UnityVersionControlParsers::ParseRepository(Results);
	}
...
static bool RunStatus(const FString& InDir, TArray<FString>&& InFiles, const EStatusSearchType InSearchType, TArray<FString>& OutErrorMessages, TArray<FUnityVersionControlState>& OutStates, int32& OutChangeset)
{
	...
	if (bResult)
	{
		...
		if (Results.Num() > 0)
		{
			UnityVersionControlParsers::GetChangesetFromWorkspaceStatus(Results, OutChangeset);
		...
		if (bWholeDirectory)
		{
			...
			UnityVersionControlParsers::ParseDirectoryStatusResult(InDir, Results, OutStates);
		}
		else
		{
			...
			UnityVersionControlParsers::ParseFileStatusResult(MoveTemp(InFiles), Results, OutStates);
		}
	}
...
static bool RunFileinfo(const bool bInWholeDirectory, const bool bInUpdateHistory, TArray<FString>& OutErrorMessages, TArray<FUnityVersionControlState>& InOutStates)
{
	...
	if (SelectedStates.Num())
	{
		...
		if (bResult)
		{
			UnityVersionControlParsers::ParseFileinfoResults(Results, SelectedStates);
		...
		if (FFileHelper::LoadFileToString(MergeProgressContent, *MergeProgressFilename))
		{
			...
			if (MergeFromIndex > INDEX_NONE)
			{
				...
				if (MergeType.FindChar(TEXT(' '), SpaceBeforeChangesetIndex))
				{
					...
					{
						UnityVersionControlParsers::FPlasticMergeConflictParser MergeConflict(Result);
...
bool RunGetShelveFiles(TArray<FUnityVersionControlChangelistState>& InOutChangelistsStates, TArray<FString>& OutErrorMessages)
{
	...
	for (FUnityVersionControlChangelistState& ChangelistState : InOutChangelistsStates)
	{
		if (ChangelistState.ShelveId != ISourceControlState::INVALID_REVISION)
		{
			...
			if (bDiffSuccessful)
			{
				bCommandSuccessful = UnityVersionControlParsers::ParseShelveDiffResult(WorkspaceRoot, MoveTemp(Results), ChangelistState);
			}
		}
	}
...
bool RunGetShelves(TArray<FUnityVersionControlChangelistState>& InOutChangelistsStates, TArray<FString>& OutErrorMessages)
{
	...
	{
		bCommandSuccessful = UnityVersionControlParsers::ParseShelvesResults(Results, InOutChangelistsStates);
		if (bCommandSuccessful)
		{
...
bool RunGetShelveFiles(const int32 InShelveId, TArray<FUnityVersionControlRevision>& OutBaseRevisions, TArray<FString>& OutErrorMessages)
{
	...
	if (InShelveId != ISourceControlState::INVALID_REVISION)
	{
		...
		if (bDiffSuccessful)
		{
			bCommandSuccessful = UnityVersionControlParsers::ParseShelveDiffResults(WorkspaceRoot, MoveTemp(Results), OutBaseRevisions);
		}
	}
...
bool RunGetShelve(const int32 InShelveId, FString& OutComment, FDateTime& OutDate, FString& OutOwner, TArray<FUnityVersionControlRevision>& OutBaseRevisions, TArray<FString>& OutErrorMessages)
{
	...
	if (bCommandSuccessful)
	{
		bCommandSuccessful = UnityVersionControlParsers::ParseShelvesResult(Results, OutComment, OutDate, OutOwner);
		if (bCommandSuccessful)
		{
...
bool RunMergeBranch(const FString& InBranchName, TArray<FString>& OutUpdatedFiles, TArray<FString>& OutErrorMessages)
{
	...
	Parameters.Add(FString::Printf(TEXT("\"br:%s\""), *InBranchName));
	bResult = UnityVersionControlUtils::RunCommand(TEXT("merge"), Parameters, TArray<FString>(), InfoMessages, OutErrorMessages);
	if (bResult)
	{
		// Load and parse the result of the update command
		FString Results;
...
void RemoveRedundantErrors(FUnityVersionControlCommand& InCommand, const FString& InFilter)
{
	...
	InCommand.ErrorMessages.RemoveAll(UnityVersionControlParsers::FRemoveRedundantErrors(InFilter));
